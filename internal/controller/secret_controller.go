...
func (r *SecretReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	for _, tlsLabelType := range []string{MonitoringTLSSecretValue, AnalyticsTLSSecretValue, HealthCheckerTLSSecretValue} {
		if labelValue == tlsLabelType {
			...
			targetNamespace, ok := secret.Annotations["targetNamespace"]
			if !ok {
				targetNamespace = secret.Namespace
				log.V(1).Info("No targetNamespace annotation found on Secret, defaulting to Secret namespace", "namespace", targetNamespace)
			}

			throwerOptions := r.ThrowerOptions
			throwerOptions.TargetNamespace = targetNamespace

			throwerAppBuilder, err := builder.AppBuilderFactory(labelValue, secret.Name, secret.Namespace, targetValue, throwerOptions)
			if err != nil {
