...
var _ = Describe("Location Controller", func() {
	Context("When reconciling a resource", func() {
		...
		BeforeEach(func() {
			By("creating the custom resource for the Kind Location")
			err := k8sClient.Get(ctx, typeNamespacedName, location)
		...
		AfterEach(func() {
			...
			By("Cleanup the specific resource instance Location")
			Expect(k8sClient.Delete(ctx, resource)).To(Succeed())
		})
		It("should successfully reconcile the resource", func() {
			By("Reconciling the created resource")
			controllerReconciler := &LocationReconciler{
				Client: k8sClient,
